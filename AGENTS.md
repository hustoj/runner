...
## 编码风格与命名约定
遵循 `gofmt` 默认格式，保持 Go 代码使用制表符缩进。提交前运行 `pre-commit run --all-files`，仓库已启用 `golangci-lint`（配置见 `.golangci.yml`）、`go mod tidy`、文本卫生与安全检查 hooks。包名保持小写；平台相关文件沿用 `_linux.go`、`_darwin.go` 后缀；测试文件使用 `*_test.go`。新增配置项优先放在 `runner/config.go`，同时补充默认值与校验逻辑。
