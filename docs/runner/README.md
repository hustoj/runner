...
该子目录存放运行期行为的补充参考文档，包括 syscall 参考和资源限制契约说明。

## 当前内容

- `syscalls.md`：给出通用场景和 Java 场景常见的 syscall 清单，适合快速比对语言运行时需要的额外调用。
- `resource-limits.md`：说明 `case.json` 中 CPU / Memory / MemoryReserve / Stack / Output 的运行期语义，以及硬限制和结果归类的关系。

## 适合来这里解决的问题

- 想快速回忆默认 syscall 白名单大致长什么样
- 想确认 Java 运行时为什么需要额外 syscall
- 想确认 `Memory`、`MemoryReserve` 和 `Stack` 在 Linux 上分别落到哪些限制
- 想写文档或补测试时找一个简短参考
